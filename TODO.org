* COMMENT TODO Job-spec
  ...
  #+BEGIN_SRC clojure :tangle task-spec.clj
    ...
    ;; Needs to create anonymous supertasks; e.g.
    ;; (dependencies
    ;;  <anonymous>
  ...
  #+BEGIN_SRC clojure :tangle topological-sort.clj :shebang #!/usr/bin/env clj
    ...
    (defrecord node [name children parents])
    
    (defn make-node [name]
      (node. name (atom #{}) (atom #{})))
    
    (def table
      {'join-md5-and-payload (make-node 1)
       'map-uuid-to-md5 (make-node 2)
       'map-uuid-to-payload (make-node 3)
       'make-uuid-md5s (make-node 4)})
      ...
      #_(loop [graph graph]
          (if (list? graph)
            (doseq [node graph]
              (recur node))
            (println graph)))
      #_(letfn [(plumb [parent graph]
                  (debug
                   graph
                   parent
                   (if (list? parent) (gensym) parent)
                   )
                  (if (list? graph)
                    (doseq [node graph]
                      (plumb graph node))
                    ...
                    (loop [child (get table graph)
                      ...
                      (if (not (empty? parents))
                        (let [parent (car parents)]
                          (do (swap! (:parents child)
                                     (fn [child-parents]
                                       (conj child-parents parent)))
                              (swap! (:children parent)
                                     (fn [parent-children]
                                       (conj parent-children child)))
                              (recur (car parents)
                                     (cdr parents)))))))))]
        (plumb nil graph)
        (doseq [[task node] table] (debug task node))
        #_(letfn [(sort [graph]
                  (let [sinks
                        (filter (λ [node]
