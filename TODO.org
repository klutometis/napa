...
* TODO Attach-geocode POC
    ...
    public class UUIDToMD5Mapper {
        ...
        public static class Map extends MapReduceBase
            ...
            public void map(LongWritable key,
                ...
                output.collect(new Text(UUID), new Text(gson.toJson(new MD5(MD5))));
            }
        }
  ...
  #+BEGIN_SRC sh :tangle map-uuid-to-md5.sh :shebang #!/usr/bin/env bash
    ...
    javac -cp $(hadoop classpath):lib/* -d UUIDToMD5Mapper-classes UUIDToMD5Mapper.java && \
        yes | unzip -d UUIDToMD5Mapper-classes lib/gson-*.jar &&
    ...
    public class UUIDToPayloadMapper {
        ...
        public static class Map extends MapReduceBase
            ...
            public void map(LongWritable key,
                ...
                throws IOException {
                String[] MD5AndUUID = MD5ToUUID.toString().split(",");
                UUID uuid = new UUID(MD5AndUUID[1]);
                System.out.println(new Text(uuid.uuid).toString() +
                                   new Text(gson.toJson(uuid)).toString());
                output.collect(new Text(uuid.uuid),
                               new Text(gson.toJson(uuid)));
            }
        }
  ...
  #+BEGIN_SRC sh :tangle map-uuid-to-payload.sh :shebang #!/usr/bin/env bash
    ...
    javac -cp $(hadoop classpath):lib/* -d UUIDToPayloadMapper-classes UUIDToPayloadMapper.java && \
        yes | unzip -d UUIDToPayloadMapper-classes lib/gson-*.jar &&
  ...
  #+BEGIN_SRC java :tangle MD5AndPayloadJoiner.java
    ...
    public class MD5AndPayloadJoiner {
        static Log log = LogFactory.getLog(MD5AndPayloadJoiner.class);
    
        public static class Map extends MapReduceBase
            implements Mapper<Text, Text, Text, Text> {
            public void map(Text UUID,
                            Text NotSure,
                            OutputCollector<Text, Text> output,
                            Reporter reporter)
                throws IOException {
                System.out.println("************************" + UUID.toString() + NotSure.toString());
                // String[] MD5AndUUID = MD5ToUUID.toString().split(",");
                // String UUID = MD5AndUUID[1];
